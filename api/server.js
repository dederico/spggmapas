...
// GET /secciones/progreso -> obtener progreso de trabajo por sección
  ...
  try {
    const { rows } = await pool.query(`
      select
        ...
        count(distinct id_predio) filter (where vuelta_actual >= 1) as vuelta_1,
        count(distinct id_predio) filter (where vuelta_actual >= 2) as vuelta_2,
        count(distinct id_predio) filter (where vuelta_actual >= 3) as vuelta_3
      from predios
...
// GET /secciones/resumen -> combina totales y progreso con porcentajes
  ...
  try {
    ...
    for (const row of rows) {
      ...
      resumen.push({
        ...
        vuelta_1_completados: parseInt(row.vuelta_1) || 0,
        vuelta_2_completados: parseInt(row.vuelta_2) || 0,
        vuelta_3_completados: parseInt(row.vuelta_3) || 0
      });
    }
    ...
    for (let i = 356; i <= 417; i++) {
      ...
      if (!seccionesConDatos.has(seccion) && totalesPorSeccion[seccion]) {
        resumen.push({
          ...
          porcentaje_avance: 0,
          vuelta_1_completados: 0,
          vuelta_2_completados: 0,
          vuelta_3_completados: 0
        });
      }
    }
